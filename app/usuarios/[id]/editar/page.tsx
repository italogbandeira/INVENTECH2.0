  ...
  useEffect(() => {
    async function carregarUsuario() {
      try {
        ...
        if (!response.ok) {
          throw new Error(data?.erro || "Erro ao carregar usuário.");
        }

        const usuario: Usuario | null = data.usuario ?? null;
        ...
        setLoginEmail(usuario.login_email ?? "");
        setLoginMaquina(usuario.login_maquina ?? "");
      } catch (error) {
        console.error(error);
