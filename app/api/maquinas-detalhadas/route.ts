...
export async function GET(req: Request) {
  try {
    ...
    if (numeroSerie) {
      const placeholder = addValue(`%${numeroSerie}%`);
      conditions.push(`m.numero_serie ILIKE ${placeholder}`);
    }

    if (setores.length > 0) {
      const placeholders = setores.map((item) => addValue(item)).join(", ");
      conditions.push(`s.nome IN (${placeholders})`);
    }

    if (usuarios.length > 0) {
      const placeholders = usuarios.map((item) => addValue(item)).join(", ");
      conditions.push(`u.nome IN (${placeholders})`);
    }

    if (tipos.length > 0) {
      const placeholders = tipos.map((item) => addValue(item)).join(", ");
      conditions.push(`t.nome IN (${placeholders})`);
    }

    if (modelos.length > 0) {
      const placeholders = modelos.map((item) => addValue(item)).join(", ");
      conditions.push(`mo.nome IN (${placeholders})`);
    }

    if (contratos.length > 0) {
      const placeholders = contratos.map((item) => addValue(item)).join(", ");
      conditions.push(`c.nome IN (${placeholders})`);
    }

    if (origens.length > 0) {
      const placeholders = origens.map((item) => addValue(item)).join(", ");
    ...
    const totalResult = await prisma.$queryRawUnsafe<{ total: bigint | number }[]>(
      `
